//                                     
// Edited again by RC Beveridge (24 June 2020) to operate with BMDS assembler                          
// Minor changes only:                          
//   - removed 'option binary' instruction                          
//   - added 'org $200' instruction                          
//   - replaced # with $ for hex numbers                          
//                                     
...
;                                      
; 99% of this source is taken from Paul Vervalin's PONG game. The only                          
; changes are the position of the left paddle, and the vertical line on                          
; the middle of the screen. This, to have a correct game under the MS-DOS                          
; CHIP-8 emulator. Note that the modifications are quite archaic, as they                          
; were hand-made (i.e: no assembler/disassembler).                          
;                                      
; ---------------------------------------------------------------------------                          
;                                      
...
; VF     collision detection                          
              ...
              CALL  Middle             ; Draws the vertical bar at center
...
Padl_Loop:                             
              ...
              SNE   V6, 0              ; Skip next instruction if ball not at left
              JP    Left_Side          ;
...
Draw_Score:                            
              ...
              LD    V5, 2              ; Set V5 to the Y coord. to draw 1st score char
...
Middle:                                
              LD    VB, 32             ; Set X coord. of middle line pixels
              LD    VC, 0              ; Set Y coord. of the first sprite to draw
              LD    I, MiddleLine      ; I Points to the bitmap of the 6 pixels to draw

Draw_Middle:                           
              DRW   VB, VC, 4          ; Draw 4 pixels of the line
              ADD   VC, 4              ; Increments Y of the next 4 pixels sprite
              SE    VC, 32             ; End loop if last pixels drawn
              JP    Draw_Middle        ; Else draw next pixels

              LD    VA, 0              ; Set VA to X location of horizontal lines
              LD    VB, 0              ; Set VB to Y location of upper horizontal line
              LD    VC, 31             ; Set VC to Y location of downer horizontal line
              LD    I, HorizLine       ; I points on the bitmap of these lines...

Draw_Horiz:                            
              DRW   VA, VB, 1          ; Draw 8 pixels of the first horizontal line
              DRW   VA, VC, 1          ; Draw 8 pixels of the second horizontal line
              ADD   VA, 8              ; Increments Y of the next 8 pixels sprites
              SE    VA, 64             ; End loop if last pixels drawn
              JP    Draw_Horiz         ; Else draw next pixels of the lines

              LD    I, MiddleLine      ; I point again on the middle line sprite
              LD    VA, 0              ; Set player A (left side) X coord.
              LD    VB, 32             ; Set VB to first 2*1 sprite location of vertical line
              DRW   VB, VA, 1          ; Redraw this part which wad erased before...

              RET                      ; Return to program start
                                       ; VA must be set to 0, this has already been done.
