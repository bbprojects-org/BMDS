...
RCB 18 Dec 15: Added comments back in from Sam Lantinga and JEDI versions
...
{$ENDIF}
  ...
  // Allow change flags - which audio format changes are allowed when opening a device
...
{ Get the number of available devices exposed by the current driver. Only
  valid after a successfully initializing the audio subsystem.
...
{ This function takes a source format and rate and a destination format
  ...
  -1 if the format conversion is not supported, 0 if there's no conversion
  needed, or 1 if the audio filter is set up
}
...
type
  ...
  TSDL_KeyboardEvent = record
    type_,                              // SDL_KEYDOWN or SDL_KEYUP
    timestamp,
    windowID: Uint32;                   // The window with keyboard focus, if any
    state,                              // SDL_PRESSED or SDL_RELEASED
    repeat_,                            // Non-zero if this is a key repeat
    padding2,
    padding3: UInt8;
...
type
  ...
  TSDL_MouseMotionEvent = record
    ...
    x, y,                               // X,Y coordinates, relative to window
    xrel, yrel: Sint32;                 // Relative motion in the X,Y directions
  end;

  // Mouse button event structure

  TSDL_MouseButtonEvent = record
    type_,                              // SDL_MOUSEBUTTONDOWN or SDL_MOUSEBUTTONUP
    timestamp,
    windowID,                           // The window with mouse focus, if any
    ...
    clicks,
    padding1: UInt8;
    x, y: Sint32;                       // X,Y coordinates, relative to window
  end;

  // Mouse wheel event structure

  TSDL_MouseWheelEvent = record
    type_,                              // SDL_MOUSEWHEEL
    timestamp,
    windowID,                           // The window with mouse focus, if any
    which: Uint32;                      // The mouse instance id, or SDL_TOUCH_MOUSEID
  ...
  TSDL_JoyBallEvent = record
    ...
    padding2,
    padding3: UInt8;
    xrel, yrel: Sint16;                 // The relative motion in the X,Y directions
  end;
 ...
 TSDL_AudioDeviceEvent = record
  ...
  TSDL_SysWMEvent = record
    type_,                              // SDL_SYSWMEVENT
    timestamp: Uint32;
    msg: pointer;                       // Driver dependent data (defined in SDL_syswm.h)
  end;
...
{ Polls for currently pending events
    ...
    event:  If not nil, the next event is removed from the queue and
            stored in that area
    Result: 1 if there are any pending events, or 0 if there are none available
}
...
{ Waits indefinitely for the next available event
    ...
    event:  If not nil, the next event is removed from the queue and
            stored in that area
    Result: 1, or 0 if there was an error while waiting for events
}
...
{ Add an event to the event queue
    ...
    Result: 1 on success, 0 if the event was filtered, or -1 if the event queue
